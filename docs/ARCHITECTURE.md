...
```mermaid
flowchart LR
  Browser[Next.js web application] -->|HTTPS and x-api-key| Gateway[NestJS API gateway]
...
The browser never calls the Python service. The API gateway owns authorization, job state, object metadata, storage grants, request validation, idempotency, and public API versioning. FastAPI accepts only the internal service key and processes either a bounded multipart upload or a presigned storage job.
...
All public business routes are under `/api/v1` and require `x-api-key`; liveness and readiness probes are public. Validation strips no unknown data silently: unknown DTO fields are rejected. Fastify provides the NestJS HTTP adapter, with Helmet, CORS allowlisting, request body limits, and rate limiting configured during bootstrap.
