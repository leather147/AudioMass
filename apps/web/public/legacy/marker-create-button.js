(function (w, d) {
	...
	function install(app) {
		...
		function openDialog() {
			if (!canCreateMarker()) {
				if (w.OneUp) w.OneUp('Нет клипов для маркера', 1200);
				return;
			}

			var mid = 'mrk_add_named';
			var defaultName = 'Маркер';
			new PKSimpleModal({
				title: 'Новый маркер',
				...
				buttons: [{
					...
					callback: function (modal) {
						...
						if (!name) {
							if (w.OneUp) w.OneUp('Введите название маркера', 1200);
							return;
						}
				...
				body: '<label for="k_new_mrkr">Название маркера</label>' +
					'<p class="pk_marker_name_hint">Маркер будет создан в текущей позиции курсора.</p>' +
		...
		function makeButton() {
			...
			btn.title = 'Создать новый маркер';
			btn.setAttribute('aria-label', 'Создать новый маркер');
			btn.innerHTML = markerIcon() + '<span>Маркер</span>';
