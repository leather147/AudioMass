type UiFxRuntimeValue = ReturnType<typeof JSON.parse>;

(() => {
  const runtimeGlobal: UiFxRuntimeValue = globalThis;
  const window: UiFxRuntimeValue = runtimeGlobal.window;
  const document: UiFxRuntimeValue = runtimeGlobal.document;
  const PKAudioEditor: UiFxRuntimeValue = runtimeGlobal.PKAudioEditor;
  const PKSimpleModal: UiFxRuntimeValue = runtimeGlobal.window.AMLateRuntimeValue('PKSimpleModal');
  const PKAudioFXModal: UiFxRuntimeValue =
    runtimeGlobal.window.AMLateRuntimeValue('PKAudioFXModal');
  const OneUp: UiFxRuntimeValue = runtimeGlobal.OneUp;
  const WaveSurfer: UiFxRuntimeValue = runtimeGlobal.WaveSurfer;
  const dragNDrop: UiFxRuntimeValue = runtimeGlobal.window.AMLateRuntimeValue('dragNDrop');
  const ID3v2: UiFxRuntimeValue = runtimeGlobal.window.AMLateRuntimeValue('ID3v2');
  const ID4: UiFxRuntimeValue = runtimeGlobal.window.AMLateRuntimeValue('ID4');
  const wasm_denoise_stream_perf: UiFxRuntimeValue = runtimeGlobal.window.AMLateRuntimeValue(
    'wasm_denoise_stream_perf',
  );
  const app: UiFxRuntimeValue = PKAudioEditor;
  (function (
    this: UiFxRuntimeValue,
    w?: UiFxRuntimeValue,
    d?: UiFxRuntimeValue,
    PKAE?: UiFxRuntimeValue,
  ) {
    'use strict';
    function translate(this: UiFxRuntimeValue, value?: UiFxRuntimeValue) {
      return w.AMI18n ? w.AMI18n.t(value) : value;
    }
    function PK_FX_PRESETS(this: UiFxRuntimeValue) {
      var presets: UiFxRuntimeValue = {};
      function persist(this: UiFxRuntimeValue) {
        if (w.AMPreferences) w.AMPreferences.set('effectPresets', presets);
      }
      this.Set = function (
        this: UiFxRuntimeValue,
        filter_id?: UiFxRuntimeValue,
        obj?: UiFxRuntimeValue,
      ) {
        var arr: UiFxRuntimeValue = presets[filter_id];
        if (!arr) {
          arr = [];
      ...
      this.Save = function (this: UiFxRuntimeValue) {
        persist();
      };
      this.Get = function (this: UiFxRuntimeValue, filter_id?: UiFxRuntimeValue) {
      ...
      this.GetSingle = function (
        this: UiFxRuntimeValue,
        filter_id?: UiFxRuntimeValue,
        custom_id?: UiFxRuntimeValue,
      ) {
        if (!filter_id) return false;
        if (!custom_id) return false;
        var arr: UiFxRuntimeValue = presets[filter_id];
        var l: UiFxRuntimeValue = arr.length;
        var found: UiFxRuntimeValue = null;
        while (l-- > 0) {
      ...
      this.Del = function (
        ...
        var arr: UiFxRuntimeValue = presets[filter_id];
        var l: UiFxRuntimeValue = arr.length;
        var found: UiFxRuntimeValue = false;
        while (l-- > 0) {
          if (arr[l].id === custom_id) {
            arr.splice(l, 1);
            found = true;
      ...
      if (!w.AMPreferences) {
        this.Set = function (this: UiFxRuntimeValue) {};
        return;
      }
      var saved: UiFxRuntimeValue = w.AMPreferences.get('effectPresets', null);
      if (saved) presets = saved;
    }
    function PKUI_FX(this: UiFxRuntimeValue, app?: UiFxRuntimeValue) {
      var UI: UiFxRuntimeValue = app.ui;
      var curr_filter_ui: UiFxRuntimeValue = null;
      var modal_name: UiFxRuntimeValue = 'modalfx';
      var modal_esc_key: UiFxRuntimeValue = modal_name + 'esc';
      var custom_presets: UiFxRuntimeValue = new (PK_FX_PRESETS as UiFxRuntimeValue)();
      function activeCursor(this: UiFxRuntimeValue) {
        return UI.GetActiveCursor
          ? UI.GetActiveCursor()
          : PKAudioEditor.engine.wavesurfer.getCurrentTime();
      }
      app.listenFor('DidCloseFX_UI', function (this: UiFxRuntimeValue) {
        curr_filter_ui = null;
      });
      app.listenFor('DidOpenFX_UI', function (this: UiFxRuntimeValue, modal?: UiFxRuntimeValue) {
        curr_filter_ui = modal;
      });
      app.listenFor('RequestFXUI_SELCUT', function (this: UiFxRuntimeValue) {
        var eng: UiFxRuntimeValue = app.engine;
        var wv: UiFxRuntimeValue = eng.wavesurfer;
        var bk: UiFxRuntimeValue = wv.backend;
        var rate: UiFxRuntimeValue = bk.buffer.sampleRate;
        var region: UiFxRuntimeValue = wv.regions.list[0];
        if (!region) return false;
        app.fireEvent('RequestPause');
        // mark the region as
        region.element.style.background = 'red';
        var reg: UiFxRuntimeValue = {
          pos: {
        ...
        wv.backend.reg = reg;
        var update_reg: UiFxRuntimeValue = function (
          this: UiFxRuntimeValue,
          region?: UiFxRuntimeValue,
        ) {
      ...
      app.listenFor('RequestFXUI_Gain', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'gain';
        var auto: UiFxRuntimeValue = null;
        var getvalue: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var value: UiFxRuntimeValue;
          if (auto) {
            value = auto.GetValue();
          } else {
            var input: UiFxRuntimeValue = q.el_body.getElementsByTagName('input')[0];
            value = [{ val: input.value / 1 }];
          }
          return value;
        };
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            id: filter_id,
            ...
            ondestroy: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              app.ui.InteractionHandler.on = false;
              app.ui.KeyHandler.removeCallback(modal_esc_key);
            },
            preview: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var value: UiFxRuntimeValue = getvalue(q);
            ...
            buttons: [
              {
                ...
                callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                  var value: UiFxRuntimeValue = getvalue(q);
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var range: UiFxRuntimeValue = q.el_body.getElementsByTagName('input')[0];
              var span: UiFxRuntimeValue = q.el_body.getElementsByTagName('span')[0];
              var graph_btn: UiFxRuntimeValue = q.el_body.getElementsByTagName('a')[0];
              range.oninput = function (this: UiFxRuntimeValue) {
              ...
              app.ui.KeyHandler.addCallback(
                modal_esc_key,
                function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
      ...
      app.listenFor('RequestActionFXUI_Rate', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'speed';
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            id: filter_id,
            title: 'Change Speed',
            ...
            preview: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var input: UiFxRuntimeValue = q.el_body.getElementsByTagName('input')[0];
              var value: UiFxRuntimeValue = input.value.trim() / 1;
      ...
      app.listenFor('RequestActionFXUI_Speed', function (this: UiFxRuntimeValue) {
        var mt: UiFxRuntimeValue = app.multitrack;
        ...
        var getvalue: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var input: UiFxRuntimeValue = q.el_body.getElementsByTagName('input')[0];
          var points: UiFxRuntimeValue = auto && auto.GetValue()[0];
        ...
        var profile_points: UiFxRuntimeValue = function (
          this: UiFxRuntimeValue,
          val?: UiFxRuntimeValue,
        ) {
          var raw: UiFxRuntimeValue = val.substring(8).split('|');
          var points: UiFxRuntimeValue = [];
          for (var i: UiFxRuntimeValue = 0; i < raw.length; ++i) {
            var pair: UiFxRuntimeValue = raw[i].split(':');
        ...
        var draw_axis: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
          if (!auto || !auto.act) return;
          var ctx: UiFxRuntimeValue = auto.ctx;
          var min: UiFxRuntimeValue = auto.act.min;
          var max: UiFxRuntimeValue = auto.act.max;
          var vals: UiFxRuntimeValue = [max, 1, min];
          var pts: UiFxRuntimeValue = auto.points[auto.act.id] || [];
          ctx.save();
          ctx.font = '10px Arial';
          ctx.textBaseline = 'middle';
          var y1: UiFxRuntimeValue = (1 - (1 - min) / (max - min)) * auto.ch;
          ctx.strokeStyle = 'rgba(217,217,85,0.35)';
          ctx.beginPath();
          ctx.moveTo(0, y1);
          ctx.lineTo(auto.cw, y1);
          ctx.stroke();
          for (var i: UiFxRuntimeValue = 0; i < vals.length; ++i) {
            var y: UiFxRuntimeValue = (1 - (vals[i] - min) / (max - min)) * auto.ch;
            var txt: UiFxRuntimeValue = vals[i].toFixed(2) + 'x';
            var w: UiFxRuntimeValue = ctx.measureText(txt).width + 8;
            if (y < 8) y = 8;
          ...
          ctx.fillStyle = '#fff';
          for (var i: UiFxRuntimeValue = 0; i < pts.length; ++i) {
            var txt: UiFxRuntimeValue = pts[i].val.toFixed(2) + 'x';
            var w: UiFxRuntimeValue = ctx.measureText(txt).width;
            var x: UiFxRuntimeValue = pts[i].ax + 9;
            var y: UiFxRuntimeValue = pts[i].ay - 10;
        ...
        var setpoints: UiFxRuntimeValue = function (
          this: UiFxRuntimeValue,
          q?: UiFxRuntimeValue,
          points?: UiFxRuntimeValue,
        ) {
          if (!auto || !points.length) return;
          var input: UiFxRuntimeValue = q.el_body.getElementsByTagName('input')[0];
          var min: UiFxRuntimeValue = input.min / 1;
          var max: UiFxRuntimeValue = input.max / 1;
          var duration: UiFxRuntimeValue = auto.wv.getDuration();
          var region: UiFxRuntimeValue = auto.wv.regions.list[0];
          ...
          for (var i: UiFxRuntimeValue = 0; i < points.length; ++i) {
            var xval: UiFxRuntimeValue = Math.max(0, Math.min(1, points[i].x / 1));
            var yval: UiFxRuntimeValue = Math.max(min, Math.min(max, points[i].val / 1));
            var y: UiFxRuntimeValue = 1 - (yval - min) / (max - min);
          ...
          auto.points[input.id].sort(function (
            this: UiFxRuntimeValue,
            a?: UiFxRuntimeValue,
            b?: UiFxRuntimeValue,
          ) {
        ...
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            ...
            onpreset: function (this: UiFxRuntimeValue, val?: UiFxRuntimeValue) {
              var range: UiFxRuntimeValue = x.el_body.getElementsByTagName('input')[0];
              var span: UiFxRuntimeValue = x.el_body.getElementsByTagName('span')[0];
              if ((val + '').indexOf('profile:') === 0) {
                var points: UiFxRuntimeValue = profile_points(val);
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              ...
              q.waveDarken = 0.45;
              auto = new PKAudioEditor._deps.FxAUT(app, q, null, function (
                this: UiFxRuntimeValue,
                p?: UiFxRuntimeValue,
              ) {
                var r: UiFxRuntimeValue = auto.wv.regions.list[0],
                  d: UiFxRuntimeValue = r ? r.end - r.start : auto.wv.getDuration();
                app.fireEvent('RequestActionFX_PREVIEW_SPEED', { val: getvalue(q), seek: p * d });
              });
              var pc: UiFxRuntimeValue = d.createElement('canvas'),
                px: UiFxRuntimeValue = pc.getContext('2d'),
                pp: UiFxRuntimeValue = 0,
                pt: UiFxRuntimeValue = 0,
                po: UiFxRuntimeValue = 0;
              q.el_body.style.position || (q.el_body.style.position = 'relative');
              pc.width = auto.cw;
              pc.height = auto.ch;
              pc.style.cssText = 'position:absolute;pointer-events:none;z-index:3';
              q.el_body.appendChild(pc);
              function pcn(this: UiFxRuntimeValue) {
                return (
                  (w.performance && w.performance.now ? w.performance.now() : Date.now()) / 1000
                );
              }
              function pcd(this: UiFxRuntimeValue) {
                var r: UiFxRuntimeValue = auto.wv.regions.list[0];
                return r ? r.end - r.start : auto.wv.getDuration();
              }
              function pcl(this: UiFxRuntimeValue, x?: UiFxRuntimeValue) {
                px.clearRect(0, 0, pc.width, pc.height);
                if (x >= 0) {
                  px.fillStyle = '#ff3355';
                  px.fillRect((x * pc.width) >> 0, 0, 2, pc.height);
                }
              }
              function pcr(this: UiFxRuntimeValue, x?: UiFxRuntimeValue) {
                var p: UiFxRuntimeValue = auto.act && auto.points[auto.act.id],
                  i: UiFxRuntimeValue = 1,
                  a: UiFxRuntimeValue,
                  b: UiFxRuntimeValue;
              ...
              function pct(this: UiFxRuntimeValue) {
                var n: UiFxRuntimeValue = pcn(),
                  du: UiFxRuntimeValue = pcd();
                if (!po || !(du > 0)) return;
                pp = (pp + (n - pt) * pcr(pp / du)) % du;
                pt = n;
                pcl(pp / du);
                q._spRAF = requestAnimationFrame(pct);
              }
              q._spStart = function (this: UiFxRuntimeValue, seek?: UiFxRuntimeValue) {
                var du: UiFxRuntimeValue = pcd();
                if (!(du > 0)) return;
                po = 1;
                pp = Math.max(0, Math.min(seek || 0, du - 0.001));
                pt = pcn();
                pct();
              };
              q._spStop = function (this: UiFxRuntimeValue) {
                po = 0;
                if (q._spRAF) cancelAnimationFrame(q._spRAF);
                q._spRAF = 0;
                pcl(-1);
              };
              app.listenFor('DidStartPreview', q._spStart);
              app.listenFor('DidStopPreview', q._spStop);
              auto.btn_auto.style.display = 'none';
              var rm: UiFxRuntimeValue = d.createElement('a');
              rm.className = 'pk_modal_a_bottom';
              rm.innerHTML = 'Delete';
              rm.style.cssText = 'display:none;position:absolute;z-index:4;margin:0';
              q.el_body.appendChild(rm);
              function rmb(this: UiFxRuntimeValue) {
                var p: UiFxRuntimeValue = auto.act_point,
                  a: UiFxRuntimeValue = auto.act && auto.points[auto.act.id];
                if (!p || !a || a.length < 3) return (rm.style.display = 'none');
                rm.style.display = 'block';
                rm.style.left =
                  auto.canvas.offsetLeft + auto.canvas.offsetWidth - rm.offsetWidth - 6 + 'px';
                rm.style.top =
                  auto.canvas.offsetTop + auto.canvas.offsetHeight - rm.offsetHeight - 6 + 'px';
              }
              rm.onclick = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
                e.preventDefault();
                auto.DelAct(2) && app.fireEvent('RequestActionFX_UPDATE_PREVIEW', getvalue(q));
              };
              var render: UiFxRuntimeValue = auto.Render;
              auto.Render = function (this: UiFxRuntimeValue) {
                render.call(auto);
                draw_axis();
      ...
      app.listenFor('RequestActionFXUI_Flip', function (this: UiFxRuntimeValue) {
        if (!PKAudioEditor.engine.is_ready) return;
        app.fireEvent('RequestRegionClear');
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'flip';
        var mode: UiFxRuntimeValue = 0;
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            id: filter_id,
            title: 'Channel Info',
            ...
            buttons: [
              {
                ...
                callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                  if (mode === 1) {
                    ...
                    var mono: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_c_mm')[0];
                    var flip: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_c_fl')[0];
                    if (mono.checked) {
                      var chans: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_c_c');
                    ...
                    var stereo: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_c_ms')[0];
                    if (stereo.checked) {
            ...
            body:
              ...
              '</div>',
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var main: UiFxRuntimeValue = null;
              var num: UiFxRuntimeValue =
                PKAudioEditor.engine.wavesurfer.backend.buffer.numberOfChannels;
              if (num === 2) {
                mode = 1;
                main = q.el_body.getElementsByClassName('pk_mm')[0];
                var mono: UiFxRuntimeValue = main.getElementsByClassName('pk_c_mm')[0];
                var flip: UiFxRuntimeValue = main.getElementsByClassName('pk_c_fl')[0];
                var chans: UiFxRuntimeValue = main.getElementsByClassName('pk_c_c');
                var tmp: UiFxRuntimeValue = main.getElementsByClassName('pk_dis');
                var lbls: UiFxRuntimeValue = [tmp[0], tmp[1]];
                mono.onchange = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
                  if (mono.checked) {
                    flip.checked = false;
                ...
                flip.onchange = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
                  if (flip.checked) {
                    mono.checked = false;
                    mono.onchange();
                  }
      ...
      app.listenFor('RequestFXUI_Silence', function (this: UiFxRuntimeValue) {
        var x: UiFxRuntimeValue = new PKSimpleModal({
          ...
          buttons: [
            {
              ...
              clss: 'pk_modal_a_accpt',
              callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                var input: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_horiz')[0];
                var value: UiFxRuntimeValue = input.value.trim() / 1;
                var radios: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_check');
                var offset: UiFxRuntimeValue = 0;
                if (radios[1].checked) offset = activeCursor().toFixed(3) / 1;
          ...
          setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
            var cursor_pos_el: UiFxRuntimeValue = q.el_body.getElementsByClassName('pkcdpk')[0];
            cursor_pos_el.innerHTML = activeCursor().toFixed(2) + 's';
            var range: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_horiz')[0];
            var span: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_val')[0];
            range.oninput = function (this: UiFxRuntimeValue) {
      ...
      app.listenFor('RequestActionFXUI_Compressor', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'compressor';
        var auto: UiFxRuntimeValue = null;
        var getvalue: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var ret: UiFxRuntimeValue;
          var value: UiFxRuntimeValue = [];
          if (auto) {
            value = auto.GetValue();
          } else {
            var inputs: UiFxRuntimeValue = q.el_body.getElementsByTagName('input');
        ...
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            ...
            preview: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var inputs: UiFxRuntimeValue = q.el_body.getElementsByTagName('input');
              var val: UiFxRuntimeValue = getvalue(q);
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var inputs: UiFxRuntimeValue = q.el_body.getElementsByTagName('input');
              for (var i: UiFxRuntimeValue = 0; i < inputs.length; ++i) {
                (function (this: UiFxRuntimeValue, k?: UiFxRuntimeValue) {
                  inputs[k].oninput = function (this: UiFxRuntimeValue) {
                    var v: UiFxRuntimeValue = +this.value;
                    this.nextElementSibling.firstChild.nodeValue =
                      k === 5 ? v.toFixed(1) + ' dB' : v.toFixed(3);
                    updateFilter();
                  };
                })(i);
              }
              ...
              //};
              function updateFilter(this: UiFxRuntimeValue) {
                var val: UiFxRuntimeValue = getvalue(q);
              ...
              var gF: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_grmtr_f')[0];
              var gV: UiFxRuntimeValue =
                q.el_body.getElementsByClassName('pk_grmtr_v')[0].firstChild;
              var lr: UiFxRuntimeValue = NaN;
              function tickGR(this: UiFxRuntimeValue) {
                q._grRAF = requestAnimationFrame(tickGR);
                var h: UiFxRuntimeValue = app.engine && app.engine.FXPreviewHost,
                  n: UiFxRuntimeValue = h && h.PreviewFilter,
                  r: UiFxRuntimeValue = 0;
                if (h && h.MTPreviewing && h.MTPreviewFilter) n = h.MTPreviewFilter;
                if (h && (h.previewing || h.MTPreviewing) && n) {
                  var c: UiFxRuntimeValue = n.length ? n[0] : n;
                  r = c.reduction;
                  if (typeof r !== 'number') r = (r && r.value) || 0;
                }
                if (r === lr) return;
                lr = r;
                var p: UiFxRuntimeValue = -r * 5;
                if (p > 100) p = 100;
                else if (p < 0) p = 0;
                gF.style.width = p + '%';
      ...
      app.listenFor('RequestActionFXUI_Normalize', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var getMode: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          return q._normMode || 'peak';
        };
        var getTarget: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var target: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_target')[0];
          var custom: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_custom_v')[0];
          return target.value === 'custom' ? custom.value / 1 : target.value.split('|')[1] / 1;
        };
        var getCeiling: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var target: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_target')[0];
          var ceiling: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_ceil')[0];
          return target.value === 'custom' ? ceiling.value / 1 : target.value.split('|')[2] / 1;
        };
        var getValue: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var mode: UiFxRuntimeValue = getMode(q);
        ...
        var fireFor: UiFxRuntimeValue = function (
          this: UiFxRuntimeValue,
          mode?: UiFxRuntimeValue,
          preview?: UiFxRuntimeValue,
        ) {
          if (mode === 'lufs')
            return preview
        ...
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            ...
            body:
              '<div class="pk_row pk_norm_modes"><label>Mode</label>' +
              ...
              '</div>',
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var modes: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_norm_mode');
              var range: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_norm_amt')[0];
              var span: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_val')[0];
              var simple: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_norm_simple')[0];
              var lufs: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_norm_lufs')[0];
              var target: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_target')[0];
              var custom: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_custom')[0];
              var customVal: UiFxRuntimeValue =
                q.el_body.getElementsByClassName('pk_lufs_custom_v')[0];
              var ceiling: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_ceil')[0];
              var out: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_lufs_out')[0];
              var customOut: UiFxRuntimeValue = custom.getElementsByClassName('pk_val')[0];
              var ceilingOut: UiFxRuntimeValue =
                ceiling.parentNode.getElementsByClassName('pk_val')[0];
              var equally: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_norm_eq')[0];
              var applyBtn: UiFxRuntimeValue = q.els.bottom[0];
              function fmt(this: UiFxRuntimeValue, v?: UiFxRuntimeValue) {
                return isFinite(v) ? v.toFixed(1) : '-inf';
              }
              function signed(this: UiFxRuntimeValue, v?: UiFxRuntimeValue) {
                return (v > 0 ? '+' : '') + fmt(v);
              }
              function targetName(this: UiFxRuntimeValue) {
                return target.options[target.selectedIndex].text.replace(/\s*\(.*/, '');
              }
              function syncCeiling(this: UiFxRuntimeValue) {
                ceiling.value = getCeiling(q);
                ceilingOut.innerHTML = (ceiling.value / 1).toFixed(1) + ' dBTP';
              }
              function syncApply(this: UiFxRuntimeValue) {
                if (q._normMode === 'lufs') {
              ...
              function previewOn(this: UiFxRuntimeValue) {
                return (
                  q.els &&
                  q.els.toolbar &&
                  q.els.toolbar[1] &&
              ...
              function refreshPreview(this: UiFxRuntimeValue) {
                if (!previewOn()) return;
                clearTimeout(q._normPreviewTimer);
                q._normPreviewTimer = setTimeout(function (this: UiFxRuntimeValue) {
                  var val: UiFxRuntimeValue = getValue(q);
                  if (!previewOn() || !val) return;
                  app.fireEvent('RequestActionFX_UPDATE_PREVIEW', val);
                }, 60);
              }
              function restartPreview(this: UiFxRuntimeValue) {
                if (!previewOn()) return;
                clearTimeout(q._normPreviewTimer);
                var val: UiFxRuntimeValue = getValue(q);
                if (!val) return;
                app.fireEvent('RequestActionFX_PREVIEW_STOP');
                setTimeout(function (this: UiFxRuntimeValue) {
                  if (q.els) app.fireEvent(fireFor(getMode(q), true), val);
                }, 20);
              }
              function setSimpleMode(this: UiFxRuntimeValue) {
                if (q._normMode === 'rms') {
                  range.min = -40;
                  range.max = 0;
                  range.step = 0.1;
              ...
              function updateLufs(this: UiFxRuntimeValue) {
                if (!q._lufsReport) return;
                var norm: UiFxRuntimeValue = app._deps.lufs.gainForTarget(
                  q._lufsReport,
                  getTarget(q),
                  ceiling.value / 1,
                );
                ...
                out.innerHTML =
                  'Source: ' +
                  fmt(q._lufsReport.lufs) +
                  ' LUFS, est. TP ' +
                  fmt(q._lufsReport.truePeakDb) +
                  ' dBTP<br>' +
                  'After: ' +
                  fmt(norm.expectedLUFS) +
                  ' LUFS, est. TP ' +
                  fmt(norm.expectedTruePeakDb) +
                  ' dBTP<br>' +
                  '<span class="' +
                  (norm.limited ? 'pk_lufs_warn' : 'pk_lufs_note') +
                  '">' +
                  'Gain ' +
                  signed(norm.gainDb) +
                  ' dB for ' +
              ...
              function resetLufs(this: UiFxRuntimeValue) {
                q._lufsReport = null;
                q._lufsNorm = null;
              ...
              range.oninput = function (this: UiFxRuntimeValue) {
                if (q._normMode === 'rms') span.innerHTML = (range.value / 1).toFixed(1) + ' dBFS';
                else span.innerHTML = (((range.value / 1) * 100) >> 0) + '%';
                refreshPreview();
              };
              equally.onchange = restartPreview;
              function setMode(this: UiFxRuntimeValue, val?: UiFxRuntimeValue) {
                q._normMode = val;
                for (var i: UiFxRuntimeValue = 0; i < modes.length; ++i)
              ...
              for (var m: UiFxRuntimeValue = 0; m < modes.length; ++m)
                modes[m].onchange = function (this: UiFxRuntimeValue) {
                  if (this.checked) setMode(this.value);
                };
              target.onchange = function (this: UiFxRuntimeValue) {
                custom.style.display = target.value === 'custom' ? '' : 'none';
                syncCeiling();
                if (q._lufsReport) updateLufs();
                else q.AnalyzeLUFS();
              };
              customVal.oninput = function (this: UiFxRuntimeValue) {
                customOut.innerHTML = (customVal.value / 1).toFixed(1);
                if (q._lufsReport) updateLufs();
                else q.AnalyzeLUFS();
              };
              ceiling.oninput = function (this: UiFxRuntimeValue) {
                ceilingOut.innerHTML = (ceiling.value / 1).toFixed(1) + ' dBTP';
                if (q._lufsReport) updateLufs();
                else q.AnalyzeLUFS();
              };
              q.AnalyzeLUFS = function (this: UiFxRuntimeValue) {
                ...
                setTimeout(function (this: UiFxRuntimeValue) {
                  app.fireEvent(
                    ...
                    function (this: UiFxRuntimeValue, report?: UiFxRuntimeValue) {
                      q._measuring = false;
                      if (!report) {
      ...
      app.listenFor('RequestActionFXUI_ParaGraphicEQ', function (this: UiFxRuntimeValue) {
        PKAudioEditor._deps.FxEQ(app, custom_presets);
      });
      app.listenFor('RequestActionTempo', function (this: UiFxRuntimeValue) {
        PKAudioEditor._deps.FxTMP(app);
      });
      app.listenFor('RequestActionNewRec', function (this: UiFxRuntimeValue) {
      ...
      app.listenFor(
        ...
        function (this: UiFxRuntimeValue, num_of_bands?: UiFxRuntimeValue) {
          app.fireEvent('RequestSelect', 1);
          var filter_id: UiFxRuntimeValue = 'graph_eq';
          var auto: UiFxRuntimeValue = null;
          var getvalue: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            ranges?: UiFxRuntimeValue,
          ) {
            var val: UiFxRuntimeValue = {};
            if (auto) {
              val = auto.GetValue();
            } else {
              val = [];
              var len: UiFxRuntimeValue = ranges.length;
              for (var i: UiFxRuntimeValue = 0; i < len; ++i) {
                var range: UiFxRuntimeValue = ranges[i];
                val.push({
          ...
          var bands_str: UiFxRuntimeValue =
          ...
          var band_q: UiFxRuntimeValue = 4.6;
          if (num_of_bands === 20) {
            filter_id += '_2';
            presets = [
              ...
              {
                name: 'Telephone',
                val: '-25,-25,-25,-20,-15,-8,-2,0,2,3,4,3,2,0,-2,-8,-15,-25,-25,-25',
              },
              {
                name: 'Old Radio',
                val: '-25,-25,-22,-20,-18,-12,-9,-4,0,4,8,9,10,5,-8,-15,-20,-25,-25,-25',
              },
          ...
          var x: UiFxRuntimeValue = new PKAudioFXModal(
            {
              ...
              preview: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                var ranges: UiFxRuntimeValue = q.el_body.getElementsByTagName('input');
                var len: UiFxRuntimeValue = ranges.length;
              ...
              setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                ...
                for (var i: UiFxRuntimeValue = 0; i < len; ++i) {
                  var range: UiFxRuntimeValue = ranges[i];
                  range.oninput = function (this: UiFxRuntimeValue) {
                    var span: UiFxRuntimeValue = this.parentNode.getElementsByTagName('span')[0];
      ...
      app.listenFor('RequestActionFXUI_HardLimiter', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            title: 'Hard Limiting',
            ...
            updateFilter: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var val: UiFxRuntimeValue = [q.el_body.getElementsByClassName('pk_check')[0].checked];
              var ranges: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_horiz');
              for (var i: UiFxRuntimeValue = 0; i < ranges.length; ++i) {
                var range: UiFxRuntimeValue = ranges[i];
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var ranges: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_horiz');
              ranges[0].oninput = function (this: UiFxRuntimeValue) {
                var span: UiFxRuntimeValue = this.parentNode.getElementsByTagName('span')[0];
                span.innerHTML = (((this.value / 1) * 100) >> 0) + '%';
                app.fireEvent('RequestActionFX_UPDATE_PREVIEW', q.updateFilter(q));
              };
              ranges[1].oninput = function (this: UiFxRuntimeValue) {
                var span: UiFxRuntimeValue = this.parentNode.getElementsByTagName('span')[0];
                span.innerHTML = 'Ratio ' + (((this.value / 1) * 100) >> 0) + '%';
                app.fireEvent('RequestActionFX_UPDATE_PREVIEW', q.updateFilter(q));
              };
              ranges[2].oninput = function (this: UiFxRuntimeValue) {
                var span: UiFxRuntimeValue = this.parentNode.getElementsByTagName('span')[0];
      ...
      app.listenFor('RequestActionFXUI_Delay', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'delay';
        var auto: UiFxRuntimeValue = null;
        var getvalue: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var ret: UiFxRuntimeValue;
          var value: UiFxRuntimeValue = [];
        ...
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var inputs: UiFxRuntimeValue = q.el_body.getElementsByTagName('input');
              for (var i: UiFxRuntimeValue = 0; i < inputs.length; ++i) {
                inputs[i].oninput = function (this: UiFxRuntimeValue) {
                  var span: UiFxRuntimeValue = this.parentNode.getElementsByTagName('span')[0];
      ...
      app.listenFor('RequestActionFXUI_Distortion', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'dist';
        var auto: UiFxRuntimeValue = null;
        var getvalue: UiFxRuntimeValue = function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var value: UiFxRuntimeValue;
          if (auto) {
      ...
      app.listenFor('RequestActionFXUI_Reverb', function (this: UiFxRuntimeValue) {
        app.fireEvent('RequestSelect', 1);
        var filter_id: UiFxRuntimeValue = 'reverb';
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            id: filter_id,
      ...
      app.listenFor('RequestActionFXUI_Repair', function (this: UiFxRuntimeValue) {
        var mt: UiFxRuntimeValue = app.multitrack;
        if (mt && mt.IsOn && mt.IsOn())
          return OneUp('Audio Repair is not available in multitrack', 1400);
        if (!app.engine.is_ready) return;
        app.fireEvent('RequestSelect', 1);
        function mode(this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          return q.el_body.querySelector('input[name=rpr_m]:checked').value;
        }
        function getvalue(this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
          var m: UiFxRuntimeValue = mode(q);
          if (m === 'hum') {
            var f: UiFxRuntimeValue = q.el_body.querySelector('input[name=rpr_f]:checked');
            return f ? f.value : 'auto';
          }
          var sel: UiFxRuntimeValue = q.el_body.querySelector(
            'input[name=' + (m === 'edit' ? 'rpr_e' : 'rpr_s') + ']:checked',
          );
          return sel ? sel.value : 'med';
        }
        var EVT: UiFxRuntimeValue = {
        ...
        var x: UiFxRuntimeValue = new PKAudioFXModal(
          {
            id: 'repair',
            title: 'Audio Repair',
            ondestroy: function (this: UiFxRuntimeValue) {
              app.ui.InteractionHandler.on = false;
              app.ui.KeyHandler.removeCallback(modal_esc_key);
            },
            preview: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              ...
              q.el_body.querySelectorAll('input[name=rpr_m]').forEach(function (
                this: UiFxRuntimeValue,
                r?: UiFxRuntimeValue,
              ) {
              ...
              app.ui.KeyHandler.addCallback(
                modal_esc_key,
                function (this: UiFxRuntimeValue) {
                  if (!app.ui.InteractionHandler.check(modal_name)) return;
                  q.Destroy();
                },
                [27],
      ...
      app.listenFor('RequestActionFXUI_SeamlessLoop', function (this: UiFxRuntimeValue) {
        if (!app.engine.is_ready) return;
        ...
        var x: UiFxRuntimeValue = new PKSimpleModal({
          ...
          body:
            '<canvas class="pk_sloop" width="520" height="96"></canvas>' +
          ...
          setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
            var canvas: UiFxRuntimeValue = q.el_body.getElementsByTagName('canvas')[0];
            var ctx: UiFxRuntimeValue = canvas.getContext('2d', { alpha: false, antialias: false });
            var cache: UiFxRuntimeValue = d.createElement('canvas');
            cache.width = canvas.width;
            cache.height = canvas.height;
            var cctx: UiFxRuntimeValue = cache.getContext('2d', { alpha: false, antialias: false });
            var inputs: UiFxRuntimeValue = q.el_body.getElementsByTagName('input');
            var span: UiFxRuntimeValue = q.el_body.getElementsByTagName('span')[0];
            var meta: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_sloop_meta')[0];
            var region: UiFxRuntimeValue = app.engine.wavesurfer.regions.list[0];
            var buffer: UiFxRuntimeValue = app.engine.wavesurfer.backend.buffer;
            var start: UiFxRuntimeValue = (region.start * buffer.sampleRate) >> 0;
            var len: UiFxRuntimeValue = ((region.end - region.start) * buffer.sampleRate) >> 0;
            var end: UiFxRuntimeValue = start + len;
            var playing: UiFxRuntimeValue = 0;
            var dirty: UiFxRuntimeValue = 1,
              dur: UiFxRuntimeValue = 0;
            var now: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
              return (w.performance && w.performance.now ? w.performance.now() : Date.now()) / 1000;
            };
            var repeat: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
              return Math.max(1, Math.min(64, inputs[1].value >> 0 || 1));
            };
            var zero: UiFxRuntimeValue = function (
              this: UiFxRuntimeValue,
              data?: UiFxRuntimeValue,
              i?: UiFxRuntimeValue,
              dir?: UiFxRuntimeValue,
              max?: UiFxRuntimeValue,
              slope?: UiFxRuntimeValue,
            ) {
              var any: UiFxRuntimeValue = -1;
              for (var n: UiFxRuntimeValue = 0; n < max; ++n, i += dir) {
            ...
            function edges(this: UiFxRuntimeValue) {
              var s: UiFxRuntimeValue = start,
                e: UiFxRuntimeValue = end,
                data: UiFxRuntimeValue = buffer.getChannelData(0);
              if (inputs[2].checked) {
                var pad: UiFxRuntimeValue = (buffer.sampleRate / 1000) >> 0;
              ...
              if (inputs[3].checked) {
                var max: UiFxRuntimeValue = Math.min((buffer.sampleRate / 100) >> 0, (e - s) >> 3);
                var z1: UiFxRuntimeValue = max > 2 ? zero(data, s + 1, 1, max, 0) : -1;
                var slope: UiFxRuntimeValue = z1 > 0 ? data[z1 + 1] - data[z1 - 1] : 0;
                var z2: UiFxRuntimeValue = z1 > 0 ? zero(data, e - 2, -1, max, slope) : -1;
                if (z2 > z1 + 8) {
                  s = z1;
                  e = z2 + 1;
                }
            ...
            function draw(this: UiFxRuntimeValue, pos?: UiFxRuntimeValue) {
              var w: UiFxRuntimeValue = canvas.width,
                h: UiFxRuntimeValue = canvas.height;
              if (dirty) {
                var b: UiFxRuntimeValue = edges(),
                  l: UiFxRuntimeValue = b[1] - b[0];
                if (l < 1) {
                  dur = 0;
                  return;
                }
                app.engine.GetWave(buffer, w, h, b[0], b[1], cache, cctx);
                var f: UiFxRuntimeValue = Math.min(
                  ((inputs[0].value * buffer.sampleRate) / 1000) >> 0,
                  l >> 2,
                );
                var fx: UiFxRuntimeValue = f ? Math.max(2, ((f / l) * w) >> 0) : 0;
                cctx.fillStyle = 'rgba(255,179,92,.22)';
                cctx.fillRect(0, 0, fx, h);
                cctx.fillRect(w - fx, 0, fx, h);
                if (fx > 1) {
                  var data: UiFxRuntimeValue = buffer.getChannelData(0);
                  cctx.strokeStyle = '#ffb35c';
                  cctx.beginPath();
                  for (var x: UiFxRuntimeValue = 0; x < fx; ++x) {
                    var v: UiFxRuntimeValue = data[b[1] - f + (((x / fx) * f) >> 0)] || 0;
                    cctx[x ? 'lineTo' : 'moveTo'](x, h / 2 - (v * h) / 2);
                  }
                  cctx.stroke();
                }
                var r: UiFxRuntimeValue = repeat();
                var ol: UiFxRuntimeValue = l - f;
            ...
            function tick(this: UiFxRuntimeValue) {
              if (!playing) return;
              if (dirty) draw();
              if (dur <= 0) return;
            ...
            q._val = function (this: UiFxRuntimeValue) {
              return {
                fade: inputs[0].value / 1,
                repeat: repeat(),
                trim: inputs[2].checked,
                snap: inputs[3].checked,
              };
            };
            q._sloopStart = function (this: UiFxRuntimeValue, seek?: UiFxRuntimeValue) {
              q.els.toolbar[0].classList.add('pk_act');
              playing = now() - (seek || 0);
              tick();
            };
            q._sloopStop = function (this: UiFxRuntimeValue) {
            ...
            var updatePreview: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
              if (!playing) return;
              if (q._sloopRestart) clearTimeout(q._sloopRestart);
              q._sloopRestart = setTimeout(function (this: UiFxRuntimeValue) {
                app.fireEvent('RequestActionFX_PREVIEW_STOP');
                app.fireEvent('RequestActionFX_PREVIEW_SeamlessLoop', q._val());
              }, 35);
            };
            inputs[0].oninput = function (this: UiFxRuntimeValue) {
              span.innerHTML = inputs[0].value + ' ms';
              dirty = 1;
              draw();
              updatePreview();
            };
            inputs[1].oninput = inputs[1].onchange = function (this: UiFxRuntimeValue) {
              dirty = 1;
              draw();
              updatePreview();
            };
            inputs[2].onchange = function (this: UiFxRuntimeValue) {
              dirty = 1;
              draw();
              updatePreview();
            };
            inputs[3].onchange = function (this: UiFxRuntimeValue) {
              dirty = 1;
              draw();
              updatePreview();
            };
            canvas.onclick = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
              if (!playing) return;
              if (dirty) draw();
              if (dur <= 0) return;
              var r: UiFxRuntimeValue = canvas.getBoundingClientRect();
              var v: UiFxRuntimeValue = q._val();
      ...
      // -----
      var current_tags: UiFxRuntimeValue = null;
      app.listenFor(
        'RequestActionID3',
        function (this: UiFxRuntimeValue, flag?: UiFxRuntimeValue, new_tags?: UiFxRuntimeValue) {
          if (flag) {
            current_tags = new_tags;
            return;
          }
          var modal_id: UiFxRuntimeValue = '_id3';
          var live: UiFxRuntimeValue = true;
          var st: UiFxRuntimeValue = {
            tag: current_tags || {},
            ab: null,
          ...
          var esc: UiFxRuntimeValue = function (this: UiFxRuntimeValue, v?: UiFxRuntimeValue) {
            return (v || '')
              .toString()
              .replace(/[&<>"']/g, function (this: UiFxRuntimeValue, m?: UiFxRuntimeValue) {
                return (
                  {
                    '&': '&amp;',
                    '<': '&lt;',
          ...
          var img: UiFxRuntimeValue = function (this: UiFxRuntimeValue, image?: UiFxRuntimeValue) {
            if (!image) return '-';
            var base64str: UiFxRuntimeValue = '';
            for (var i: UiFxRuntimeValue = 0; i < image.data.length; ++i) {
              base64str += String.fromCharCode(image.data[i]);
            }
            return (
              '<img style="max-width:340px;max-height:230px" src="data:' +
              image.format +
              ';base64,' +
          ...
          var snap: UiFxRuntimeValue = function (this: UiFxRuntimeValue, t?: UiFxRuntimeValue) {
            t = t || {};
            return {
              artist: t.artist || '',
              title: t.title || '',
          ...
          var read_tags: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            ab?: UiFxRuntimeValue,
          ) {
            var b: UiFxRuntimeValue = new Uint8Array(ab),
              t: UiFxRuntimeValue = null;
            try {
            ...
            } catch (e: UiFxRuntimeValue) {}
            return t;
          };
          var dirty: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            q?: UiFxRuntimeValue,
            el?: UiFxRuntimeValue,
          ) {
            var btn: UiFxRuntimeValue = q.els.bottom[0];
            var on: UiFxRuntimeValue = st.file && st.type === 'mp3' && st.ab && st.ch;
            if (!on && st.file && st.type === 'mp3' && st.ab && st.base) {
              for (var k in st.base)
                if (st.base.hasOwnProperty(k)) {
                  var x: UiFxRuntimeValue = el.querySelector('[name="' + k + '"]');
          ...
          var render_cover: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            q?: UiFxRuntimeValue,
            el?: UiFxRuntimeValue,
            off?: UiFxRuntimeValue,
          ) {
            var box: UiFxRuntimeValue = el.getElementsByClassName('pk_id3cover')[0];
            if (!box) return;
            var ttl: UiFxRuntimeValue = 'padding-left:0';
            var str: UiFxRuntimeValue =
              '<span class="pk_id3ttl" style="' +
              ttl +
              '">Cover</span>' +
              '<div class="pk_id3img" style="margin-top:4px">' +
              img(st.pic) +
              '</div>';
            if (!off) {
              str += '<div>';
              str +=
                '<a class="pk_modal_a_bottom pk_id3picbtn" style="display:inline-block;float:none;margin:10px 8px 0 0">Change Cover</a>';
              if (st.pic) {
                str +=
                  '<a class="pk_modal_a_bottom pk_id3rm" style="display:inline-block;float:none;margin:10px 8px 0 0">Remove Image</a>';
              }
              if (st.ch) {
                str +=
                  '<a class="pk_modal_a_bottom pk_id3undo" style="display:inline-block;float:none;margin:10px 8px 0 0">Undo</a>';
              }
              str += '</div>';
            }
            box.innerHTML = str;
            ...
            var rm: UiFxRuntimeValue = box.getElementsByClassName('pk_id3rm')[0];
            if (rm)
              rm.onclick = function (this: UiFxRuntimeValue) {
                st.pic = null;
                st.ch = true;
                render_cover(q, el, off);
                dirty(q, el);
              };
            var undo: UiFxRuntimeValue = box.getElementsByClassName('pk_id3undo')[0];
            if (undo)
              undo.onclick = function (this: UiFxRuntimeValue) {
                st.pic = st.orig || null;
                st.ch = false;
                render_cover(q, el, off);
                dirty(q, el);
          ...
          var render_tags: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            q?: UiFxRuntimeValue,
            el?: UiFxRuntimeValue,
            tags?: UiFxRuntimeValue,
          ) {
            var off: UiFxRuntimeValue = st.type !== 'mp3';
            var dis: UiFxRuntimeValue = off ? ' disabled' : '';
            st.tag = tags || {};
            st.base = snap(st.tag);
            if (!st.ch) st.pic = st.tag.picture || null;
            var row: UiFxRuntimeValue =
              'display:inline-block;width:49%;box-sizing:border-box;margin:0 1% 3px 0;vertical-align:top';
            var inp: UiFxRuntimeValue = 'width:100%;box-sizing:border-box;margin-top:2px';
            var ttl: UiFxRuntimeValue = 'padding-left:0';
            var field: UiFxRuntimeValue = function (
              this: UiFxRuntimeValue,
              n?: UiFxRuntimeValue,
              l?: UiFxRuntimeValue,
              v?: UiFxRuntimeValue,
            ...
            var area: UiFxRuntimeValue = function (
              this: UiFxRuntimeValue,
              n?: UiFxRuntimeValue,
              l?: UiFxRuntimeValue,
              v?: UiFxRuntimeValue,
              ...
              return (
                '<label style="display:block;width:68%;min-width:260px;max-width:460px;margin:0 1% 3px 0"><span class="pk_id3ttl" style="' +
                ttl +
                '">' +
                l +
                '</span>' +
                '<textarea name="' +
                n +
                '" class="pk_txt" style="width:100%;height:' +
                h +
                'px;box-sizing:border-box;margin-top:2px"' +
                dis +
                '>' +
                esc(v) +
                '</textarea></label>'
            ...
            var str: UiFxRuntimeValue = '<div style="margin-top:12px;max-width:640px">';
            if (off) {
              str +=
                '<div style="padding:8px 0">Only MP3 files can be edited here. These tags are view-only.</div>';
            }
            ...
            var ins: UiFxRuntimeValue = el.querySelectorAll('input, textarea');
            for (var i: UiFxRuntimeValue = 0; i < ins.length; ++i) {
              ins[i].oninput = function (this: UiFxRuntimeValue) {
                dirty(q, el);
              };
            }
            render_cover(q, el, off);
          ...
          new PKSimpleModal({
            ...
            buttons: [
              {
                ...
                callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                  var e: UiFxRuntimeValue = function (
                    this: UiFxRuntimeValue,
                    n?: UiFxRuntimeValue,
                  ) {
                    var x: UiFxRuntimeValue = q.el_body.querySelector('[name="' + n + '"]');
                    return x ? x.value : '';
                  };
                  ...
                  var out: UiFxRuntimeValue = ID3v2.WriteTags(st.ab, {
                    artist: e('artist'),
                    title: e('title'),
                    album: e('album'),
                    year: e('year'),
                  ...
                  var url: UiFxRuntimeValue = (window.URL || window.webkitURL).createObjectURL(
                    new Blob([out], { type: 'audio/mpeg' }),
                  );
                  var a: UiFxRuntimeValue = document.createElement('a');
                  a.href = url;
                  a.download =
                    ((st.file && st.file.name) || 'audiomass.mp3').replace(/\.[^\.]+$/, '') +
                    '-tagged.mp3';
            ...
            body:
              '<input class="pk_id3file" type="file" accept="audio/*" style="display:none" />' +
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              var input: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_id3file')[0];
              var picinp: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_id3picfile')[0];
              var pick: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_id3choose')[0];
              var fname: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_id3fname')[0];
              var txt_el: UiFxRuntimeValue = q.el_body.getElementsByClassName('pk_ttx')[0];
              q.els.bottom[0].style.display = 'none';
              pick.onclick = function (this: UiFxRuntimeValue) {
                input.click();
              };
              input.onchange = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
                var reader: UiFxRuntimeValue = new FileReader();
                var file: UiFxRuntimeValue = this.files[0];
                if (!file) return;
                st.file = file;
                st.name = file.name || '';
                st.type =
                  /\.mp3$/i.test(file.name) || /mpeg|mp3/i.test(file.type)
                    ? 'mp3'
                    : /\.(m4a|mp4)$/i.test(file.name) || /mp4|m4a/i.test(file.type)
                      ? 'm4a'
                      : '';
                st.ab = null;
                st.tag = {};
                ...
                reader.onload = function (this: UiFxRuntimeValue) {
                  if (!live) return;
                  var raw: UiFxRuntimeValue = new Uint8Array(this.result);
                  var tags: UiFxRuntimeValue = read_tags(this.result);
                  if (
                    !tags &&
                    (st.type === 'm4a' || (raw[0] === 73 && raw[1] === 68 && raw[2] === 51))
                  ) {
                    txt_el.innerHTML = '<div style="padding:30px 0">Unsupported audio file.</div>';
                    return;
                  }
              ...
              picinp.onchange = function (this: UiFxRuntimeValue) {
                var file: UiFxRuntimeValue = this.files[0];
                if (!file) return;
                var reader: UiFxRuntimeValue = new FileReader();
                reader.onload = function (this: UiFxRuntimeValue) {
      ...
      // ---- save presets
      app.listenFor('RequestSavePreset', function (this: UiFxRuntimeValue) {
        if (!curr_filter_ui) return;
        var el: UiFxRuntimeValue = curr_filter_ui.el_body;
        if (!el) return;
        var escapeHtml: UiFxRuntimeValue = function (
          this: UiFxRuntimeValue,
          text?: UiFxRuntimeValue,
        ) {
          var map: UiFxRuntimeValue = {
            '&': '&amp;',
            '<': '&lt;',
            '>': '&gt;',
            '"': '&quot;',
          ...
          return text.replace(/[&<>"']/g, function (this: UiFxRuntimeValue, m?: UiFxRuntimeValue) {
            return map[m];
          });
        };
        // check if the preset is custom
        var is_new: UiFxRuntimeValue = true;
        var custom_id: UiFxRuntimeValue = null;
        var el_presets: UiFxRuntimeValue = curr_filter_ui.el_presets;
        var sel_opt: UiFxRuntimeValue = el_presets.options[el_presets.selectedIndex];
        var inputs: UiFxRuntimeValue = el.querySelectorAll('select, input');
        var preset_obj: UiFxRuntimeValue = {
        ...
        if (preset_obj.val.length > 0) {
          ...
          var modal_id: UiFxRuntimeValue = '_ctPr';
          var default_txt: UiFxRuntimeValue = '';
          var btn_delete: UiFxRuntimeValue = {};
          var btn_update: UiFxRuntimeValue = {};
          var custom_obj: UiFxRuntimeValue = null;
          if (!is_new) {
            ...
            btn_delete = {
              ...
              callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                ...
                var custom: UiFxRuntimeValue = custom_presets.Del(preset_obj.target, custom_id);
            ...
            btn_update = {
              ...
              callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                if (custom_obj) {
                  var input: UiFxRuntimeValue = q.el_body.getElementsByTagName('input')[0];
                  var value: UiFxRuntimeValue = input.value.trim();
                  value = escapeHtml(value);
                  if (value.length > 0) {
                    OneUp('Successfully updated preset!', 1400);
                    // Persist the custom preset through the shared preference service.
                    custom_obj.name = value;
                    custom_obj.val = preset_obj.val;
                    custom_presets.Save();
                    var arr: UiFxRuntimeValue = custom_presets.Get(preset_obj.target);
          ...
          var title: UiFxRuntimeValue = 'Save Custom Preset for filter "' + curr_filter_ui.id + '"';
          if (!is_new) {
            var cname: UiFxRuntimeValue = custom_obj.name;
          ...
          new PKSimpleModal({
            ...
            buttons: [
              {
                ...
                callback: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
                  ...
                  value = escapeHtml(value);
                  if (value.length > 0) {
                    OneUp('Preset saved!', 1400);
                    // Persist the custom preset through the shared preference service.
                    preset_obj.name = value;
                    var custom: UiFxRuntimeValue = custom_presets.Set(
                      preset_obj.target,
                      preset_obj,
                    );
                    app.fireEvent('DidSetPresets', preset_obj.target, custom);
                    ...
                    OneUp('The name is too short...', 1200);
                  }
                  // -
                },
              },
            ...
            body:
              ...
              '<input style="width:100%;box-sizing:border-box;min-width:0" maxlength="16" placeholder="Enter a name, for example: My preset" ' +
              default_txt +
            ...
            setup: function (this: UiFxRuntimeValue, q?: UiFxRuntimeValue) {
              ...
              setTimeout(function (this: UiFxRuntimeValue) {
                if (q.el) {
                  var inp: UiFxRuntimeValue = q.el.getElementsByTagName('input')[0];
                  inp.focus();
      ...
      // ---- windows ----
      var eq_win: UiFxRuntimeValue = {};
      var mix_dock_h: UiFxRuntimeValue = 280;
      var resizeDock: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
      ...
      app.listenFor('WillUnload', function (this: UiFxRuntimeValue) {
        var cur: UiFxRuntimeValue;
        for (var k in eq_win) {
          cur = eq_win[k];
          if (cur && !cur.type) {
      ...
      app.listenFor(
        'RequestDragI',
        function (this: UiFxRuntimeValue, url?: UiFxRuntimeValue, start?: UiFxRuntimeValue) {
          if (app.isMobile) {
            alert('unsupported on mobile');
            return;
          }
          var cur_win: UiFxRuntimeValue = eq_win[url];
          if (!cur_win || !cur_win.el) return;
          var dock: UiFxRuntimeValue = app.ui.BarBtm.el;
          ...
          var el_back: UiFxRuntimeValue = document.createElement('div');
          el_back.className = 'pk_modal_back';
          document.body.appendChild(el_back);
          var is_drag: UiFxRuntimeValue = true;
          var x: UiFxRuntimeValue = start && start[0] !== undefined ? start[0] : null;
          var y: UiFxRuntimeValue = start && start[1] !== undefined ? start[1] : null;
          var moved: UiFxRuntimeValue = 2;
          var did_undock: UiFxRuntimeValue = false;
          var drag_docs: UiFxRuntimeValue = [];
          var child_doc: UiFxRuntimeValue = cur_win.win && cur_win.win.document;
          if (child_doc && child_doc !== d) drag_docs.push(child_doc);
          var top: UiFxRuntimeValue = parseInt(cur_win.el.style.top) || 0;
          var left: UiFxRuntimeValue = parseInt(cur_win.el.style.left) || 0;
          ...
          var evPos: UiFxRuntimeValue = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
            return [
              e.screenX === undefined ? e.pageX : e.screenX,
              e.screenY === undefined ? e.pageY : e.screenY,
            ];
          };
          var atViewEdge: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            e?: UiFxRuntimeValue,
          ) {
            var win_x: UiFxRuntimeValue = window.screenLeft || window.screenX || 0;
            var win_y: UiFxRuntimeValue = window.screenTop || window.screenY || 0;
            if (
              e.screenX !== undefined &&
              (e.screenX <= win_x + 3 ||
          ...
          var outOfView: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
            var rect: UiFxRuntimeValue = cur_win.el && cur_win.el.getBoundingClientRect();
            return (
              rect &&
              (rect.left <= -3 ||
                rect.top <= -3 ||
          ...
          var move: UiFxRuntimeValue = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
            if (!is_drag) return;
            var pos: UiFxRuntimeValue = evPos(e);
            if (x === null) {
              x = pos[0];
              y = pos[1];
              return;
            }
            var dist_x: UiFxRuntimeValue = pos[0] - x;
            var dist_y: UiFxRuntimeValue = pos[1] - y;
          ...
          var up: UiFxRuntimeValue = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
            if (!el_back) return;
            is_drag = false;
            ...
            app.ui.InteractionHandler.on = false;
            for (var di: UiFxRuntimeValue = 0; di < drag_docs.length; ++di) {
              drag_docs[di].removeEventListener('mousemove', move);
              drag_docs[di].removeEventListener('mouseup', up);
            }
            if (el_back && el_back.parentNode) document.body.removeChild(el_back);
            if (e.type === 'mouseup') {
              cur_win.el.style.top = '0px';
              if (moved > 0) {
                var ch: UiFxRuntimeValue = app.ui.BarBtm.el.childNodes;
                var lw: UiFxRuntimeValue = 0;
                for (var ji: UiFxRuntimeValue = 0; ji < ch.length; ++ji) {
          ...
          var eventOutOfView: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            e?: UiFxRuntimeValue,
          ) {
            var x: UiFxRuntimeValue = e.clientX;
            var y: UiFxRuntimeValue = e.clientY;
            if (x === undefined || y === undefined) return true;
            return x <= 0 || y <= 0 || x >= window.innerWidth || y >= window.innerHeight;
          };
          var leavesWindow: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            e?: UiFxRuntimeValue,
          ) {
            return !e.relatedTarget && !e.toElement;
          };
          var screenPos: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            e?: UiFxRuntimeValue,
          ) {
            return [
          ...
          var undock: UiFxRuntimeValue = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
            if (did_undock) return;
            did_undock = true;
            var pos: UiFxRuntimeValue = screenPos(e);
            up(e);
            app.fireEvent('RequestShowFreqAn', url, [pos, 0]);
          };
          el_back.onmousemove = move;
          el_back.onmouseup = up;
          el_back.onmouseleave = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
      ...
      app.listenFor(
        'RequestShowFreqAn',
        function (this: UiFxRuntimeValue, url?: UiFxRuntimeValue, args_arr?: UiFxRuntimeValue) {
          var tool_routes: UiFxRuntimeValue = {
            eq: '/tools/frequency-analyser',
            sp: '/tools/spectral-analyser',
            mix: '/tools/multitrack-mixer',
          };
          var tool_route: UiFxRuntimeValue = tool_routes[url];
          if (!tool_route) return;
          if (app.isMobile) {
            alert(translate('This tool is not available on mobile devices yet.'));
            return;
          }
          args_arr = args_arr || [];
          var toggle: UiFxRuntimeValue = args_arr[0];
          var type: UiFxRuntimeValue = args_arr[1];
          var title: UiFxRuntimeValue = 'Frequency analysis';
          var curr_win: UiFxRuntimeValue = eq_win[url];
          var is_mix: UiFxRuntimeValue = url === 'mix';
          if (url === 'sp') title = 'Spectrum analysis';
          if (is_mix) title = 'Multitrack Mixer';
          var toggled: UiFxRuntimeValue = false;
          if (curr_win && toggle) {
            var ext: UiFxRuntimeValue = false;
          ...
          var freq_cb: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            _?: UiFxRuntimeValue,
            freq?: UiFxRuntimeValue,
          ) {
            if (is_mix) return;
            curr_win && curr_win.win && curr_win.win.update && curr_win.win.update(freq);
          };
          var setEvents: UiFxRuntimeValue = function (
            this: UiFxRuntimeValue,
            obj?: UiFxRuntimeValue,
            _url?: UiFxRuntimeValue,
          ) {
            var destroy: UiFxRuntimeValue = obj.destroy;
            var done: UiFxRuntimeValue = false;
            var cleanup: UiFxRuntimeValue = function (this: UiFxRuntimeValue) {
              if (done) return;
              done = true;
              ...
              var stop: UiFxRuntimeValue = true;
              for (var k in eq_win) {
                if (eq_win[k] && k !== 'mix') {
                  stop = false;
                  break;
            ...
            obj.destroy = function (this: UiFxRuntimeValue) {
              destroy && destroy();
              cleanup();
            };
          ...
          if (!type) {
            var makePopup: UiFxRuntimeValue = function (
              this: UiFxRuntimeValue,
              dat?: UiFxRuntimeValue,
            ) {
              var extra: UiFxRuntimeValue = '';
              if (dat && dat[0]) {
                dat[0] = Math.max(0, dat[0] - 200) >> 0;
                dat[1] = Math.max(0, dat[1]) >> 0;
                extra = ',left=' + dat[0] + ',top=' + dat[1];
              }
              var size: UiFxRuntimeValue = is_mix ? [760, 320] : [600, 188];
              var wnd: UiFxRuntimeValue = window.open(
                tool_route,
                translate(title),
                'directories=no,titlebar=no,toolbar=no,' +
                  'location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=' +
                  size[0] +
                  ',height=' +
                  size[1] +
                  extra,
              ...
              if (!wnd) {
                OneUp('Allow pop-up windows for AudioMass.', 3600, 'pk_r');
                return;
              }
              eq_win[url] = {
                type: type,
                el: null,
                win: wnd,
                destroy: function (this: UiFxRuntimeValue) {
            ...
            var iframe: UiFxRuntimeValue = document.createElement('iframe');
            ...
            curr_win = eq_win[url];
            resizeDock();
            iframe.onload = function (this: UiFxRuntimeValue, e?: UiFxRuntimeValue) {
