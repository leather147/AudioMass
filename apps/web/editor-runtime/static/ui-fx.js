(function ( w, d, PKAE ) {
	...
	function PK_FX_PRESETS () {
		...
		var saved = w.AMPreferences.get ('effectPresets', null);
			...
			if (preset_obj.val.length > 0)
			{
					...
					new PKSimpleModal({
					  ...
					  body:'<label for="k07">Preset Name</label>' +
						'<input style="width:100%;box-sizing:border-box;min-width:0" maxlength="16" placeholder="Enter a name, for example: My preset" ' + default_txt + ' class="pk_txt" type="text" id="k07" />',
					  setup:function( q ) {
								// app.fireEvent ('RequestPause');
		...
		app.listenFor ('RequestShowFreqAn', function ( url, args_arr ) {
			...
			if (!type)
			{
				var makePopup = function ( dat ) {
					...
					var wnd = window.open (tool_route, translate (title), "directories=no,titlebar=no,toolbar=no,"+
