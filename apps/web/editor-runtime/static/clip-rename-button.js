(function (w, d) {
	...
	function install(app) {
		...
		function renameTo(name) {
			name = cleanName(name);
			if (!name) {
				if (w.OneUp) w.OneUp('Enter a clip name', 1200);
				return false;
			}
			if (!canRename()) {
				if (w.OneUp) w.OneUp('Select an audio clip', 1200);
				return false;
			}
			...
			if (w.OneUp) w.OneUp('Clip renamed', 900);
			return true;
		}
		...
		function openDialog() {
			...
			new PKSimpleModal({
				...
				body: '<label for="k_clip_rename">Clip name</label>' +
		...
		function makeButton() {
			...
			btn.title = 'Rename selected audio clip';
			btn.setAttribute('aria-label', 'Rename selected audio clip');
