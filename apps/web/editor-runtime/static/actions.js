(function( PKAE ) {
	...
	function AudioUtils ( master, wavesurfer ) {
		...
		var bufferOperations = window.AMAudioBufferOperations.create (
			wavesurfer,
			function ( duration ) { master.fireEvent ('DidUpdateLen', duration); }
		);
		var OverwriteBufferWithSegment = bufferOperations.overwriteSegment;
		var OverwriteBuffer = bufferOperations.replaceBuffer;
		var MakeSilenceBuffer = bufferOperations.makeSilence;
		var CopyBufferSegment = bufferOperations.copySegment;
		...
		var TrimBuffer = bufferOperations.trim;
		var InsertSegmentToBuffer = bufferOperations.insertSegment;
		var ReplaceFloatArrays = bufferOperations.replaceFloatArrays;
		var InsertFloatArrays = bufferOperations.insertFloatArrays;
...
//		}
		...
		var effectUtilities = window.AMAudioEffectUtilities;
		var fadeCurve = function ( rev ) { return effectUtilities.fadeCurve (fadeGain, rev); };
		var gainFilter = effectUtilities.gainFilter;
		var setGainValue = effectUtilities.setGainValue;
		var channelGainFilter = effectUtilities.channelGainFilter;
		var setChannelGains = effectUtilities.setChannelGains;
		var applyBufferGains = effectUtilities.applyBufferGains;
		var peakNormalizeStats = effectUtilities.peakNormalizeStats;
		var peakNormalizeGains = effectUtilities.peakNormalizeGains;
		var rmsNormalizeStats = effectUtilities.rmsNormalizeStats;
		var rmsNormalizeGains = effectUtilities.rmsNormalizeGains;
		...
		function lufsNormalizeGain ( source, val ) {
			...
			var clampRate = effectUtilities.clampRate;
			var ratePoints = effectUtilities.ratePoints;
			var rateDuration = effectUtilities.rateDuration;
			var rateAt = effectUtilities.rateAt;
			var setRate = effectUtilities.setRate;
