(function (w, d) {
	...
	var messages = {
		...
		'AM is a browser-first audio and waveform editor':'AM — бесплатный полнофункциональный веб-редактор аудио и звуковой волны.',
		...
		'Language changes are applied instantly without reloading the editor.':'Язык меняется мгновенно без перезагрузки редактора.',
		...
		'Select Visible View':'Выделить видимую область', 'Clear':'Очистить', 'Clear Selection':'Очистить выделение',
		...
		'Marker':'Маркер', 'Add Marker':'Добавить маркер', 'Rename Marker':'Переименовать маркер',
		...
		'New marker':'Новый маркер', 'Create':'Создать', 'Enter a marker name':'Введите название маркера',
		'Marker name':'Название маркера',
		'The marker will be created at the current cursor position.':'Маркер будет создан в текущей позиции курсора.',
		...
		'Enter a clip name':'Введите название карточки', 'Select an audio clip':'Выберите звуковую карточку',
		'Clip renamed':'Карточка переименована', 'Clip name':'Название карточки',
		'The name will be shown on the selected audio clip.':'Имя будет показано на выбранной звуковой карточке.',
		'Rename selected audio clip':'Задать имя выбранной карточке звука', 'Name':'Имя',
		'Composition waveform overview':'Общий waveform композиции',
		'Focus':'Фокус', 'Normal':'Обычный', 'Window':'Окно', 'Fullscreen':'Экран',
		'Waveform view: quiet edges with the peak near the center':'Вид waveform: почти полная тишина у краёв, пик ближе к центру',
		...
		'Save as new':'Сохранить как новый', 'Preset saved!':'Пресет успешно сохранён!',
		'The name is too short...':'Имя слишком короткое...',
		'Enter a name, for example: My preset':'Введите имя, например: Мой пресет',
		'This tool is not available on mobile devices yet.':'Этот инструмент пока недоступен на мобильных устройствах.',
		'Frequency analysis':'Частотный анализ', 'Spectrum analysis':'Спектральный анализ',
		'Allow pop-up windows for AudioMass.':'Разрешите всплывающие окна для AudioMass.',
		'Welcome to AudioMass':'Добро пожаловать в AudioMass', 'Welcome to AM':'Добро пожаловать в AM',
		'AM is a free, open-source web-based audio and waveform editor.':'AM — это бесплатный веб-редактор аудио и формы волны с открытым исходным кодом.',
		'It runs entirely in your browser without a server or plug-ins!':'Он полностью работает в браузере без сервера и плагинов!',
		'Tips:':'Советы:',
		'Keep in mind that most shortcuts use the':'Большинство сочетаний клавиш используют комбинацию',
		'combination. (for example, Shift+Z is undo, Shift+C is copy, Shift+X is cut, and so on.)':'(например, Shift+Z — отмена, Shift+C — копирование, Shift+X — вырезание и так далее).',
		'See the source code on':'Посмотрите исходный код на',
		'You can load any audio format supported by your browser, edit it with fades, cuts, trims and gain changes, and apply many audio effects.':'Можно загрузить любой поддерживаемый браузером аудиоформат, применить фейды, вырезание, обрезку, изменение громкости и множество эффектов.',
		'I hope you enjoy the little music compositions. I wrote them a long time ago :)':'Надеюсь, вам понравятся небольшие музыкальные композиции. Я написал их очень давно :)'
	};
	...
	w.AMI18n.register('ru', messages, {phrases:phrases});
})(window, document);
