(function (w, d) {
	...
	function languageCard (locale, label, detail) {
		...
		card.appendChild(make('i', 'am_theme_check', '✓'));
		card.onclick = function () {
			if (w.AMI18n && w.AMI18n.setLocale(locale)) updateLanguageSelection(locale);
		};
		return card;
	}

	function updateLanguageSelection (locale) {
		if (!overlay) return;
		var cards = overlay.querySelectorAll('.am_language_card');
		for (var i = 0; i < cards.length; ++i) {
			var active = cards[i].getAttribute('data-locale') === locale;
			cards[i].classList.toggle('is-active', active);
			cards[i].setAttribute('aria-pressed', active ? 'true' : 'false');
		}
	}
	...
	function open (tab) {
		...
		languagePanel.appendChild(make('p', 'am_settings_hint', t('Language changes are applied instantly without reloading the editor.')));
	...
	w.addEventListener('am:localechange', function (event) {
		if (event.detail) updateLanguageSelection(event.detail.locale);
	});
