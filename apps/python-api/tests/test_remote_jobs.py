...
from app.schemas.jobs import RemoteInput, RemoteJobRequest, RemoteOutput
...
def test_remote_job_contract_requires_output_only_for_audio_results() -> None:
    ...
    with pytest.raises(ValidationError, match="requires an output upload grant"):
        RemoteJobRequest(operation="normalize", input=source)
...
def test_metadata_jobs_return_structured_results(
    ...
    async def fake_run(*_args: Any, **_kwargs: Any) -> FakeModelResult:
    ...
    request = RemoteJobRequest(
        ...
        input={"filename": "input.wav", "url": "https://storage.example.com/input.wav"},
    )
